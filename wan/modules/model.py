...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def _forward(
        ...
        feature_layer_set = None
        if return_features or features_only:
            if feature_layers is None:
                feature_layers = [13, 21, 29]
            feature_layer_set = set(int(layer) for layer in feature_layers)
            collected_features = []
        ...
        for ii, block in enumerate(self.blocks):
            ...
            if feature_layer_set is not None and ii in feature_layer_set:
                feature_channels = self.dim // math.prod(self.patch_size)
                collected_features.append(
                    torch.stack(self.unpatchify(x, grid_sizes, c=feature_channels))
                )
        ...
        if return_features:
            return torch.stack(x), collected_features
