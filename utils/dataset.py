...
def _clean_latent_from_array(latents, source: str):
    if latents.ndim == 5:
        return latents[-1]
    if latents.ndim == 4:
        return latents
    raise ValueError(
        f"Expected latent row from {source} to have shape "
        f"[steps, frames, channels, height, width] or [frames, channels, height, width], "
        f"got {latents.shape}."
    )
...
class LatentLMDBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        clean_latent = _clean_latent_from_array(latents, self.env.path())
        return {
            "prompts": prompts,
            "clean_latent": torch.tensor(clean_latent, dtype=torch.float32)
        }
...
class CleanLatentLMDBDataset(Dataset):
    def __init__(
        ...
        if not os.path.exists(os.path.join(data_path, "data.mdb")):
            raise FileNotFoundError(
                f"Expected a CausVid-style LMDB directory containing data.mdb: {data_path}"
            )
        self.env = lmdb.open(
            data_path,
            readonly=True,
