...
class Conv3DFeatureDiscriminator(nn.Module):
    def __init__(
        self,
        ...
        hidden_dim: int = 256,
        num_class: int = 1,
        pooled_shape=(8, 16, 16),
    ) -> None:
        super().__init__()
        self.pooled_shape = tuple(int(dim) for dim in pooled_shape)
        ...
        for _ in range(int(num_features)):
            ...
            self.heads.append(
                nn.Sequential(
                    nn.Conv3d(feature_channels, hidden_dim, kernel_size=3, padding=1),
    ...
    def forward(self, features: List[torch.Tensor]) -> torch.Tensor:
        if len(features) != len(self.heads):
            raise ValueError(
                f"Expected {len(self.heads)} discriminator features, got {len(features)}"
            )
        logits = []
        for feature, head in zip(features, self.heads):
            if feature.ndim != 5:
                raise ValueError(f"Conv3D discriminator expects [B,C,T,H,W], got {tuple(feature.shape)}")
            feature = torch.nn.functional.adaptive_avg_pool3d(feature, self.pooled_shape)
            head_dtype = next(head.parameters()).dtype
            logits.append(head(feature.to(dtype=head_dtype)))
...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def adding_cls_branch(self, atten_dim=1536, num_class=4, time_embed_dim=0) -> None:
        # Default to the original three-layer setup unless a lighter configuration is requested.
        feature_layers = getattr(self, "_gan_feature_layers", None)
        if feature_layers is None:
            feature_layers = [13, 21, 29]
        num_registers = getattr(self, "_gan_num_registers", None)
        if num_registers is None:
            num_registers = len(feature_layers)
        if num_registers != len(feature_layers):
            raise ValueError(
                f"gan_num_registers ({num_registers}) must match "
                f"len(gan_feature_layers) ({len(feature_layers)})"
            )
        gan_block_ffn_dim = getattr(self, "_gan_block_ffn_dim", 8192)
        gan_block_num_heads = getattr(self, "_gan_block_num_heads", 12)
        ...
        for _ in range(num_registers):
            block = GanAttentionBlock(
                dim=atten_dim,
                ffn_dim=gan_block_ffn_dim,
                num_heads=gan_block_num_heads,
            )
            gan_ca_blocks.append(block)
        self._gan_ca_blocks = nn.ModuleList(gan_ca_blocks)
        self._gan_ca_blocks.requires_grad_(True)
        self._gan_feature_layers = feature_layers

    def adding_conv3d_cls_branch(
        self,
        ...
        if feature_channels is None:
            feature_channels = self.model.dim // math.prod(self.model.patch_size)
        self._gan_discriminator_type = "conv3d"
        self._gan_feature_layers = feature_layers
        self._conv3d_cls_branch = Conv3DFeatureDiscriminator(
        ...
        self._conv3d_cls_branch.requires_grad_(True)
    ...
    def forward(
        ...
        discriminator_features: Optional[List[torch.Tensor]] = None,
        ...
        if discriminator_features is not None:
            if not hasattr(self, "_conv3d_cls_branch"):
                raise RuntimeError("discriminator_features requires adding_conv3d_cls_branch() first")
            logits = self._conv3d_cls_branch(discriminator_features)
            ...
            if clean_x is not None:
                ...
                if classify_mode:
                    flow_pred, logits = self.model(
                        ...
                        gan_feature_layers=self._gan_feature_layers,
        ...
        if return_features:
            return flow_pred, pred_x0, features
